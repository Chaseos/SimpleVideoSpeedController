...
if (!sourceManifest.background?.service_worker) {
  throw new Error('manifest.json must declare background.service_worker for Chromium.');
}

if (!sourceManifest.background?.scripts?.length) {
  throw new Error('manifest.json must declare background.scripts for Firefox.');
}
...
for (const target of targets) {
  ...
  const archivePath = path.join(
    distDirectory,
    `simple-video-speed-controller-${target.name}.zip`
  );
